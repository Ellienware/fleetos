...
import { createOwnerShiftAction, getOwnerDriversAction, getOwnerVehiclesAction } from '../../actions';
...
export default function NewShiftPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (dr.success) setDrivers(dr.data?.documents ?? []);
        if (vr.success) setVehicles(vr.data?.documents ?? []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const result = await createOwnerShiftAction(tenantId, {
      ...
      } catch { toast.error('Failed to schedule shift'); }
    });
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Schedule shift</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Assign a driver and vehicle to a time slot</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-5 lg:col-span-2">
            ...
            {/* Assignment */}
            <SectionCard title="Assignment" description="Select the driver and vehicle for this shift" accent="blue">
            ...
            {/* Timing */}
            <SectionCard title="Timing" description="Set the date and time for this shift" accent="blue">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                {isPending ? <><Loader2 className="h-4 w-4 animate-spin" />Saving…</> : <><Save className="h-4 w-4" />Schedule shift</>}
              </button>
              <Link
                href={`/owner/${tenantId}/shifts`}
